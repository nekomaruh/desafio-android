...
import com.example.desafio_android_accenture.data.model.OwnerModel
...
interface GithubApiClient {
    ...
    @GET("users/{user}")
    suspend fun getUser(@Path("user") user:String): Response<List<OwnerModel>>

    @GET("repos/{user}/{repo}/pulls")
    suspend fun getPullRequests(
        @Path("user") user:String,
        @Path("repo") repository: String)
    : Response<List<OwnerModel>>
}